...
- `--enable-prefill-weight-versions` adds `prefill_weight_versions`, the same span list over the prompt tokens, so KV reused from the prefix cache under older weights is visible. KV write locations and versions are captured at target forward completion and carried with the result, like routed-expert recording.
