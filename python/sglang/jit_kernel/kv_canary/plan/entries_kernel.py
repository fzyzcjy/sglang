...
def launch_plan_entries_kernel(
    *,
    ...
    max_verify_entry_tiles = _max_entry_j_tiles(
        verify_capacity=verify_capacity,
        max_seq_len_per_req=req_to_token_stride0,
    )
...
def _max_entry_j_tiles(*, verify_capacity: int, max_seq_len_per_req: int) -> int:
    effective_verify_capacity = min(verify_capacity, max_seq_len_per_req)
    return (
        effective_verify_capacity + _PLAN_VERIFY_INNER_BLOCK - 1
