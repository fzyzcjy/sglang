...
class RequestPreparer:
    ...
    async def _dispatch_mm_processor(
        ...
        if (
            ...
            return await self.raw_tokenizer_wrapper.mm_processor.process_mm_data_async(
                image_data=obj.image_data,
                audio_data=obj.audio_data,
                input_text=(input_text or input_ids),
                request_obj=obj,
    ...
    async def _batch_tokenize_and_process(
        ...
        if not RequestPreparer._batch_has_text(self, batch_size, obj):
            # All requests already have input_ids, no need to tokenize
            return [
                await RequestPreparer._tokenize_one_request(self, obj[i])
    ...
    def _should_use_batch_tokenization(self, batch_size, requests) -> bool:
        ...
        return batch_size > 0 and (
            ...
            or (
                ...
                and (not RequestPreparer._batch_has_text(self, batch_size, requests))
            )
        )
