...
async def update_weight_version(obj: UpdateWeightVersionReqInput, request: Request):
    ...
    if obj.abort_all_requests:
        _global_state.TokenizerManager.abort_request(
            tokenizer_manager.pause_controller, abort_all=True
        )
...
async def abort_request(obj: AbortReq, request: Request):
    ...
    try:
        _global_state.TokenizerManager.abort_request(
            tokenizer_manager.pause_controller, rid=obj.rid, abort_all=obj.abort_all
        )
        return Response(status_code=200)
    except Exception as e:
        return _create_error_response(e)
...
async def pause_generation(obj: PauseGenerationReqInput, request: Request):
    """Pause generation."""
    await _global_state.TokenizerManager.pause_generation(
        tokenizer_manager.pause_controller, obj
    )
    return ORJSONResponse(
...
async def continue_generation(obj: ContinueGenerationReqInput, request: Request):
    """Continue generation."""
    await _global_state.TokenizerManager.continue_generation(
        tokenizer_manager.pause_controller, obj
    )
    return ORJSONResponse(
