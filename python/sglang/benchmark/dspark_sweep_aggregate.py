...
def parse(path: str) -> list[dict]:
    ...
    for raw in open(path):
        ...
        m_cfg = re.search(r"t([01])\t(on|off)", line) or re.search(r"t([01]).*?(on|off)", line)
...
def main(path: str) -> None:
    ...
    header = f"{'dataset':<18} " + " ".join(f"t{t}/{k:<3}" for t, k in configs) + f"  {'mean':>5} {'min':>5} {'max':>5}"
    print(header)
    print("-" * len(header))
    ...
    for r in sorted(rows, key=lambda r: r["acc_len"], reverse=True):
        print(f"  {r['acc_len']:>5.2f}  {r['dataset']:<18} t{r['temp']}/{r['think']:<3} "
    ...
    for dim, key in [("temp", "temp"), ("think", "think")]:
        ...
        print(f"marginal by {dim}: " +
              ", ".join(f"{k}={statistics.fmean(v):.2f}(n{len(v)})" for k, v in sorted(groups.items(), key=str)))
