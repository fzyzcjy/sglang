...
class TestRealKvUsedPerturb(CustomTestCase):
    ...
    def test_swa_real_kv_perturb_targets_only_active_window(self) -> None:
        ...
        forward_batch = make_forward_batch(device, bs=1, seq_lens_list=(6,))
        group = _make_group(kind=PoolKind.SWA, has_real_kv=True)
...
class TestRealKvUnusedCachePerturb(CustomTestCase):
    def test_real_kv_unused_cache_flips_first_real_kv_byte_for_sweep_slot(
        self,
    ) -> None:
        """Verify real_kv_unused_cache flips only the first real KV byte for a sweep slot."""
        device = DEFAULT_DEVICE
        pool = make_pool(device, max_reqs=4, max_seq=8)
        group = _make_group(kind=PoolKind.FULL, has_real_kv=True)
        source = group.real_kv_sources_k[0]
...
def _make_group(
    *,
    ...
    if swa_index_lut is None and kind is PoolKind.SWA:
