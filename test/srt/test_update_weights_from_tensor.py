...
class TestUpdateWeightsFromTensor(unittest.TestCase):
    def test_update_weights_from_tensor(self):
        ...
        check_params_by_get_weights_by_name(engine, param_names[0], [0.0087, -0.0214, -0.0004, 0.0039, 0.0110])
        ...
        for param_name in param_names[:3]:
            check_params_by_get_weights_by_name(engine, param_name, [1.5] * 5)

        engine.shutdown()


def check_params_by_get_weights_by_name(engine, param_name, expect_values):
    actual_values_full = torch.tensor(engine.get_weights_by_name(param_name))
    print(f'check_params_by_get_weights_by_name {actual_values_full=}')
    actual_values = actual_values_full[0, :5]
